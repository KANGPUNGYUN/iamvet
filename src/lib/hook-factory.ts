...
import { AuthResponse, SocialLoginRequest, SocialLoginResponse } from '@/types/auth';
...
export function createApiHook<TData, TVariables>(
  ...
  return {
    // Query hook
    useQuery: function(variables?: TVariables, queryOptions?: UseQueryOptions<TData>) {
      return useQuery({
...
export function createSocialLoginHook(provider: 'google' | 'kakao' | 'naver') {
  return function useSocialLogin(userType: 'veterinarian' | 'hospital' | 'veterinary-student') {
    const baseUrl = typeof window !== 'undefined' 
      ? (process.env.NEXT_PUBLIC_BASE_URL || window.location.origin)
      : 'http://localhost:3000';

    const initiateLogin = (onSuccess?: (data: SocialLoginResponse) => void, onError?: (error: string) => void) => {
