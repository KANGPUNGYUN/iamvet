 ...
 */
export function expandExperienceCategories(selectedExperiences: string[]): string[] {
 ...
 * @returns 매칭 여부
 */
export function isExperienceMatching(jobExperience: string[], candidateExperience: string): boolean {
  // 구직자 경험을 기준으로 해당하는 모든 구인 조건 확인
  const expandedCandidateExperience = expandExperienceCategories([candidateExperience]);
  
  // 구인공고의 요구사항 중 하나라도 구직자의 확장된 경험과 일치하면 매칭
  return jobExperience.some(reqExp => expandedCandidateExperience.includes(reqExp));
}
 ...
 */
export function getExperienceFilterConditions(selectedExperiences: string[]): string[] {
