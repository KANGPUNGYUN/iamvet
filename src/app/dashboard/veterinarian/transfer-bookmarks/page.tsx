...
export default function VeterinarianTransferBookmarksPage() {
    ...
    try {
      ...
      if (result.status === "success") {
        ...
        if (likedTransferIds.length > 0) {
          console.log(
            "[VeterinarianTransferBookmarks] 서버에서 받은 좋아요 양도양수:",
            likedTransferIds
          );
  ...
  const handleLike = async (transferId: string) => {
    ...
    console.log(
      `[VeterinarianTransferBookmarks Like] ${transferId} - 현재 상태: ${
    ...
    try {
      ...
      console.log(
        `[VeterinarianTransferBookmarks Like] API 요청: ${method} /api/transfers/${transferId}/like`
      );
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          ...
          if (result.message?.includes("이미 좋아요한")) {
            console.log(
              `[VeterinarianTransferBookmarks Like] 서버에 이미 좋아요가 존재함. 상태를 동기화`
            );
