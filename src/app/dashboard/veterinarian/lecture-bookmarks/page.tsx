...
export default function VeterinarianLectureBookmarksPage() {
    ...
    try {
      ...
      if (result.status === "success") {
        ...
        if (likedLectureIds.length > 0) {
          console.log(
            "[VeterinarianLectureBookmarks] 서버에서 받은 좋아요 강의:",
            likedLectureIds
          );
  ...
  const handleLectureLike = async (lectureId: string) => {
    ...
    console.log(
      `[VeterinarianLectureBookmarks Like] ${lectureId} - 현재 상태: ${
    ...
    try {
      ...
      console.log(
        `[VeterinarianLectureBookmarks Like] API 요청: ${method} /api/lectures/${lectureId}/like`
      );
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          ...
          if (result.message?.includes("이미 좋아요한")) {
            console.log(
              `[VeterinarianLectureBookmarks Like] 서버에 이미 좋아요가 존재함. 상태를 동기화`
            );
