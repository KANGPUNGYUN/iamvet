  ...
  try {
    ...
    if (search || transferType || status || location) {
      countResult = await sql`
        ...
        WHERE 1=1
          ${
            search
              ? sql`AND (t.title ILIKE ${searchPattern} OR t.location ILIKE ${searchPattern} OR t.description ILIKE ${searchPattern})`
              : sql``
          }
          ${
            transferType
    ...
    const transfers = transfersResult.map((row) => {
      ...
      const getTransferType = (category: string) => {
        if (category?.includes("양도") || category?.includes("매매"))
          return "양도";
        if (category?.includes("양수") || category?.includes("인수"))
          return "양수";
        return "양도"; // 기본값
      };
      ...
      return {
        ...
        location:
          row.location || `${row.sido || ""} ${row.sigungu || ""}`.trim(),
    ...
    console.error("양도양수 목록 조회 실패:", error);
    return NextResponse.json(
