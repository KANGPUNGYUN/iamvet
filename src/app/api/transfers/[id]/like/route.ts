...
export const POST = withAuth(async (
  ...
  try {
    ...
    if (!transfer) {
      ...
      return NextResponse.json(createErrorResponse('양수양도 게시글을 찾을 수 없습니다.'), { status: 404 });
    }

    console.log('Transfer found:', transfer.id);

    const existingLike = await prisma.transferLike.findUnique({
      where: {
        userId_transferId: {
          userId: user.userId,
    ...
    if (existingLike) {
      console.log('Already liked by user:', user.userId, 'transfer:', transferId);
      return NextResponse.json(createErrorResponse('이미 좋아요한 양수양도 게시글입니다.'), { status: 400 });
    }

    console.log('Creating new like...');
    await prisma.transferLike.create({
      data: {
        userId: user.userId,
        transferId: transferId
      }
...
export const DELETE = withAuth(async (
  ...
  try {
    ...
    if (!existingLike) {
      console.log('No existing like found for user:', user.userId, 'transfer:', transferId);
      return NextResponse.json(createErrorResponse('좋아요하지 않은 양수양도 게시글입니다.'), { status: 400 });
    }

    console.log('Deleting like...');
    await prisma.transferLike.delete({
      where: {
        userId_transferId: {
          userId: user.userId,
          transferId: transferId
