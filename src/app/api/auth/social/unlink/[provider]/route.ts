...
export const DELETE = withAuth(async (request: NextRequest, { params }: RouteContext) => {
  try {
    ...
    const socialAccount = currentUser.socialAccounts?.find((acc: any) => acc.provider === provider);
    ...
    const remainingSocialAccounts = currentUser.socialAccounts.filter((acc: any) => acc.provider !== provider);
    const hasPassword = currentUser.passwordHash;

    if (remainingSocialAccounts.length === 0 && !hasPassword) {
      return NextResponse.json(
        createErrorResponse("최소 하나의 로그인 방법이 필요합니다. 비밀번호를 설정하거나 다른 소셜 계정을 연결해주세요"),
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      createApiResponse("success", "소셜 계정 연결이 해제되었습니다", {
        ...
        remainingProviders: updatedUser.socialAccounts?.map((acc: any) => acc.provider) || [],
      })
    );
  } catch (error) {
