      ...
      if (response.data.status === "success") {
        ...
        const transformedJob = {
          ...
          deadline: job.recruitEndDate 
            ? `D-${Math.max(0, Math.ceil((new Date(job.recruitEndDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24)))}` 
            : '상시채용',
          location: job.hospital_address || '위치 정보 없음',
          workConditions: {
            workType: Array.isArray(job.workType) ? job.workType.join(', ') : job.workType?.[0] || '정규직',
            workDays: (() => {
              ...
              if (Array.isArray(job.workDays)) {
                ...
                return job.workDays.map((day: string) => dayMap[day] || day).join(', ') + '요일';
              }
              return job.workDays;
            })(),
            workHours: job.workStartTime && job.workEndTime 
          ...
          qualifications: {
            education: Array.isArray(job.education) ? job.education.join(', ') : job.education?.[0] || '학력무관',
            certificates: Array.isArray(job.certifications) ? job.certifications.join(', ') : job.certifications?.[0] || '수의사 면허',
