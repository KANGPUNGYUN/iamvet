...
export const incrementJobViewCount = async (
  ...
  // 중복 조회 방지를 위해 view_logs 테이블 사용
  const checkQuery = `
    SELECT id FROM view_logs 
    WHERE target_type = 'job' AND target_id = $1 AND ip_address = $2 
    AND DATE(created_at) = CURRENT_DATE
  `;

  const checkResult = await pool.query(checkQuery, [jobId, userIp]);
  ...
  if (checkResult.rows.length === 0) {
    ...
    try {
      ...
      // 조회 로그 추가
      await pool.query(
        "INSERT INTO view_logs (target_type, target_id, ip_address) VALUES ($1, $2, $3)",
        ["job", jobId, userIp]
      );
...
export const incrementResumeViewCount = async (
  ...
  const checkQuery = `
    SELECT id FROM view_logs 
    WHERE target_type = 'resume' AND target_id = $1 AND ip_address = $2 
    AND DATE(created_at) = CURRENT_DATE
  `;

  const checkResult = await pool.query(checkQuery, [veterinarianId, userIp]);

  if (checkResult.rows.length === 0) {
    await pool.query("BEGIN");

    try {
      // 조회수는 별도 테이블에서 관리하거나 veterinarians 테이블에 컬럼 추가 필요
      // 여기서는 view_logs 테이블만 업데이트
      await pool.query(
        "INSERT INTO view_logs (target_type, target_id, ip_address) VALUES ($1, $2, $3)",
        ["resume", veterinarianId, userIp]
      );
...
export const incrementLectureViewCount = async (
  ...
  const checkQuery = `
    SELECT id FROM view_logs 
    WHERE target_type = 'lecture' AND target_id = $1 AND ip_address = $2 
    AND DATE(created_at) = CURRENT_DATE
  `;

  const checkResult = await pool.query(checkQuery, [lectureId, userIp]);
